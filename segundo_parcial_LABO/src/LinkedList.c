...
int ll_sort_dosCampos(LinkedList* this, int (*pFunc1)(void* ,void*), int order1 , int (*pFunc2)(void* ,void*), int order2 )
{
    ...
    if(this!=NULL && pFunc1!=NULL && (order1==1 || order1==0 ) && pFunc2!=NULL && (order2==1 || order2==0 ))
    {
    	for(i=0;i<ll_len(this)-1;i++)
		{
			...
			for(j=i+1;j<ll_len(this);j++)
			{
				...
				condicion1=pFunc1(pElementoi, pElementoj);
				condicion2=pFunc2(pElementoi, pElementoj);
				// condicion=1 >> mayor el 1
				// condicion=-1 >> menor el 1
				// condicion=0 >> iguales
				// ORDER=1 >> ASCENDENTE    >>> ORDENA CUANDO ES MAYOR EL 1 >> COND=1
				// ORDER=0 >> DESCENDENTE	>>> ORDENA CUANDO ES MENOR EL 1 >> COND=-1
       			...
       			if((condicion1==-1 && order1==0) ||
       				(condicion1==1 && order1==1) ||
					(condicion1==0 && condicion2==-1 && order2==0) ||
					(condicion1==0 && condicion2==1 && order2==1))
     			{
       				auxElemento= pElementoi;
					ll_set(this,i,pElementoj);
					ll_set(this,j,auxElemento);
...
pasárselos a la función “fn”. La función “fn” devolverá 1 si ese ítem se debe agregar a la lista resultado o 0 si
no debe agregarse. La función “ll_filter” generará la nueva lista resultado, agregará a la misma los ítems
correspondientes y la devolverá.
 */
...
LinkedList* ll_filter(LinkedList* this, int (*pFunc)(void* element))
{
	...
	if(this!=NULL && pFunc!=NULL)
	{
		...
		if(clone!=NULL)
		{
			...
			{
				pElemento= ll_get(this, i);
				considerar =pFunc(pElemento);
				if(considerar==1)
				{
					ll_add(clone,pElemento);
				}
...
/*
 * Crear una versión de la función ll_filter (que se llamará ll_filter_parametro) que
recibirá como parámetro una cadena de caracteres que será utilizada como parte
del criterio. Utilizar según corresponda.
 */

LinkedList* ll_filter_parametro(LinkedList* this, char* eleccion_1, int (*pFunc)(void* element, char* eleccion))
{
	int i, considerar;
	void* pElemento=NULL;
	LinkedList* clone=NULL;
	...
	if(this!=NULL && pFunc!=NULL)
	{
		...
		if(clone!=NULL)
		{
			...
			{
				pElemento= ll_get(this, i);
				considerar =pFunc(pElemento, eleccion_1);
				if(considerar==1)
				{
					ll_add(clone,pElemento);
				}
