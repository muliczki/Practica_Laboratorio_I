...
ePiloto* piloto_newParametros(char* id_piloto, char* nombre_piloto, char* apellido_piloto)
{
	ePiloto* pAux=NULL;

	if(nombre_piloto!=NULL && id_piloto!=NULL && apellido_piloto!=NULL)
	{
		pAux=piloto_new();
		...
		if(pAux!=NULL)
		{
			...
			piloto_setId(pAux, atoi(id_piloto));
			piloto_setNombre(pAux, nombre_piloto);
			piloto_setApellido(pAux, apellido_piloto);
...
int piloto_setApellido(ePiloto* this,char* apellido)
{
    ...
    if(this!=NULL && apellido!=NULL)
    {
        ...
        strcpy(this->apellido_piloto , apellido);
        ret = 0;
    }
...
int piloto_getApellido(ePiloto* this,char* apellido)
{
    ...
    if(this!=NULL && apellido!=NULL)
    {
        ...
        strcpy(apellido,this->apellido_piloto);
        ret = 0;
    }
...
int piloto_mostrarUnPiloto (void* auxiliar)
{
	...
	*/
	...
	piloto_getApellido(pAux, apellido);
...
int piloto_filtroPorNombre(void* e1, char* nombre)
{
	int valor=-10; //numero no razonable para indicar que es nulo
	char nombre_piloto[60];

	if(e1!=NULL && nombre!=NULL)
	{
		ePiloto* dato1 = (ePiloto*)e1;

		piloto_getNombre(dato1, nombre_piloto);

		if(strcmp(nombre, nombre_piloto)==0)
		{
			valor=1;
		}else{
			valor=0;
