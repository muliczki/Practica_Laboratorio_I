...
int controller_Uliczki_ListFinal(LinkedList* this1, LinkedList* this2)
{
	int ret=-1; //opcion "ERROR" para la funcion "analizar situacion" en el main
	int i, j;
	char  fecha[500], destino[500], nombre[60], apellido[60];
		...
		for(i=0; i<ll_len(this2) ;i++)
		{
			...
			for(j=0; j<ll_len(this1) ;j++)
			{
				...
				if(id_piloto== id)
				{
					...
					piloto_getApellido(auxPiloto, apellido);
					printf("%10d - %10d - %20s - %20s - %20s - %20s - %10d - %15d - %15d\n", id_vuelo, id_avion, nombre,apellido, fecha, destino,pasajeros,hora_despegue, hora_llegada);
					break;
				}
...
void controller_Uliczki_ElegirPiloto(LinkedList* this, char* nombreElegido)
{
	...
	if(this!=NULL && nombreElegido!=NULL)
	{
		...
		for(i=0; i<ll_len(this); i++)
		{
			...
			if(id_elegido == id)
			{
				piloto_getNombre(auxPiloto, nombreElegido);
				estado=0;
				break;
			}
		}
